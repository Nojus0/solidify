...
const MetaProvider: Component<{ tags?: Array<TagDescription> }> = (props) => {
  ...
  onMount(() => {
    ...
    Array.prototype.forEach.call(ssrTags, (ssrTag: Node) =>
  ...
  return (
    <MetaContext.Provider value={actions}>
      {props.children}
...
export function renderTags(tags: Array<TagDescription>) {
  return tags
    .map((tag) => {
      ...
      const props = keys
        .map((k) => (k === "children" ? "" : ` ${k}="${tag.props[k]}"`))
        .join("");
...
export const Style: Component<JSX.StyleHTMLAttributes<HTMLStyleElement>> = (
  props
) => MetaTag("style", props);

export const Meta: Component<JSX.MetaHTMLAttributes<HTMLMetaElement>> = (
  props
) => MetaTag("meta", props);
...
export const Base: Component<JSX.BaseHTMLAttributes<HTMLBaseElement>> = (
  props
) => MetaTag("base", props);
